#!/usr/bin/python
import os, re, urllib.request, urllib.parse, urllib.error, json, hashlib
...
class TagResolver:
    ...
    def get_tags_for_branch(self, filter_regexp, branch):
        filtered_tags = [x for x in self.tags if None != re.match(filter_regexp, x) and Version(x).is_valid()]
...
class ImageBuilder:
    ...
    def build_dockerfile(self, template, output, filter_regexp, branch, variables):
        parent_tags = [x for x in self.parent_repo_tags if None != re.match(filter_regexp, x)]
