...
class TagResolver:
    ...
    def get_tags_for_branch(self, filter_regexp, branch):
        filtered_tags = [x for x in self.tags if re.match(filter_regexp, x) is not None and Version(x).is_valid()]
        filtered_tags.sort(key=cmp_to_key(Version.compare))
...
class ImageBuilder:
    ...
    def build_dockerfile(self, template, output, filter_regexp, branch, variables):
        parent_tags = [x for x in self.parent_repo_tags if re.match(filter_regexp, x) is not None]

        parent_tags.sort(key=cmp_to_key(Version.compare))

        tag_resolver = TagResolver(parent_tags)
        possible_tags = tag_resolver.get_tags_for_branch(filter_regexp, branch)
        highest_base_tag = Version.highest(list(possible_tags.keys()), stable_only=False)
    ...
    def build_images(self, template, output, filter_regexp, branch, variables, test_script, limit=None):
        ...
        for base_tag, inherited_tags in list(possible_tags.items()):
            ...
            for inherited_tag in inherited_tags:
                ...
                build_status_code = os.system(
                    "docker build -f " + output + " . -t \"mdobak/php-common-stack:" + inherited_tag + "\"")
            ...
            if is_something_pushed and 0 < limit <= pushed_images_count:
                break
...
class Repository:
    def __init__(self, name):
        repo_response = urllib.request.urlopen("https://registry.hub.docker.com/v1/repositories/" + name + "/tags")
        self.tags = [x["name"] for x in json.loads(repo_response.read())]
